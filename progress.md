...
- 2026-09-25: Landing review fixes: `/push/subscribe` answers 403 in the demos and their board has no push banner, so a demo can't take a device's subscription from a real building; the showcase write-route test reads Hono's `app.routes` (named export `app`); dropped `scripts/seed-demo.ts` (demos are created on first visit; README explains local reset).
