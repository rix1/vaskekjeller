...
- 2026-09-25: Signup review fixes: the recovery reset now swaps the admin password and recovery hash in one guarded UPDATE (a double-submitted reset wins once; the winner keeps the session and sees the new code). Removed the overview invite card and the settings "Del med beboere og admins" row; the share step is only the last signup step.
