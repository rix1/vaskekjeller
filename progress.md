...
- 2026-09-23: README verification list mentions the server-rendered toast tests.
- 2026-09-23: Closing a timed toast fades it out (own `toast-leave` keyframes instead of retiming `toast-out`); status toasts rendered with a full page load are repeated in the polite live region.
...
- 2026-09-23: Live-validated toasts on `wrangler dev` + headless Chrome (JS on and scripts disabled): placement, 4 s / 8 s / sticky-error timing, Angre + hover pause, × fade, 3-toast cap, network errors, admin toasts, and page-load live-region announcements. Push "enabled" toast still unverified (headless denies notifications).
- 2026-09-23: Docs/lint pass on the toasts branch: README toast paragraph and code comments match the final code; typecheck clean.
