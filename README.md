...
calendar-week date strip, the read-only past-day view, apartment selection, waitlist counts and comment pushes,
and the server-rendered toasts (Angre confirmation, errors). It also compiles the service worker and runs it as a
classic script, since `/sw.js` is registered without `{ type: "module" }`.
